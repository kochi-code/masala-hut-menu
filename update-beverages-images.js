...
const targetCategories = ["Shakes", "Falooda", "Cool Beverages"];

async function run() {
  const snapshot = await db.collection('menuItems').where('category', 'in', targetCategories).get();
  ...
  for (const doc of snapshot.docs) {
    ...
    const imagePath = `images/categories/cool-beverages/${imageName}`;
  ...
  console.log("Done updating Beverage images!");
}
