...
async function run() {
  const snapshot = await db.collection('menuItems').where('category', '==', 'Meals').get();
  ...
  for (const doc of snapshot.docs) {
    ...
    const imagePath = `images/categories/meals/${imageName}`;
  ...
  console.log("Done updating Meals images!");
}
