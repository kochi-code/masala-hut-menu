...
async function run() {
  const snapshot = await db.collection('menuItems').where('category', '==', 'Hot Beverages').get();
  ...
  for (const doc of snapshot.docs) {
    ...
    const imagePath = `images/categories/hot-beverages/${imageName}`;
  ...
  console.log("Done updating Hot Beverages images!");
}
