...
async function run() {
  const snapshot = await db.collection('menuItems').where('category', '==', 'Fish Curry').get();
  ...
  for (const doc of snapshot.docs) {
    ...
    const imagePath = `images/categories/fish-curries/${imageName}`;
  ...
  console.log("Done updating Fish Curries images!");
}
